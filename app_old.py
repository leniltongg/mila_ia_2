...
#     cursor.execute("SELECT id, nome FROM Ano_escolar")
#     Ano_escolar = cursor.fetchall()
#     print(f"[DEBUG] Ano Escolars encontradas: {Ano_escolar}")
...
#     ]
#     print("[DEBUG] MESES_NOMES[int(mes)]carregados.")
...
def editar_campo_simulado(simulado_id):
    if current_user.tipo_usuario_id not in [5, 6]:  # Verifica se é professor
...
def portal_professores():
    ...
    simulados_tratados = [
        (
            ...
            simulado[3],  # se.nome (Ano_escolar)
...
@login_required
def enviar_simulado():
    if current_user.tipo_usuario_id not in [5, 6]:  # Garantir que apenas a Secretaria de Educação acesse
        flash("Acesso não autorizado.", "danger")
        return redirect(url_for("secretaria_educacao.portal_secretaria_educacao"))
...
#     if current_user.tipo_usuario_id not in [3, 6]:  # Apenas professores podem avaliar
#         return redirect(url_for("home"))
...
def ver_relatorio_secretaria():
    if current_user.tipo_usuario_id not in [5, 6]:  # Secretaria de Educação
        flash("Acesso não autorizado.", "danger")
        return redirect(url_for("home"))
...
def relatorio_escolas():
    if current_user.tipo_usuario_id not in [5, 6]:  # Verifica se é secretaria de educação
        flash("Acesso não autorizado.", "danger")
        return redirect(url_for("index"))
    ...
    for row in cursor.fetchall():
        if row[0] not in melhores_alunos:  # Se ainda não temos um melhor aluno para esta série
            melhores_alunos[row[0]] = {
                'ano_escolar': row[1].replace('Ano Escolar', 'Ano'),
                'nome': row[2],
                'media': row[3]
            }
    ...
    alunos_por_Ano_escolar = {row[0]: {'total': row[1], 'ativos': row[2]} for row in cursor.fetchall()}
    ...
    return render_template('relatorio_escolas.html', 
                         escolas=escolas,
                         Ano_escolar=Ano_escolar.values(),
...
def relatorio_escolas_excel():
    ...
    for Ano_escolar in Ano_escolar.values():
        ws = wb.create_sheet(title=Ano_escolar['nome'])
        ...
        # Dados da série
        for row, escola in enumerate(Ano_escolar['escolas'], 2):
            ws.cell(row=row, column=1, value=f"{row-1}º")
            ws.cell(row=row, column=2, value=escola['nome'])
            ws.cell(row=row, column=3, value=escola['total_alunos'])
...
def relatorio_escolas_pdf():
    ...
    Ano_escolar_dados = cursor.fetchall()
    ...
    for Ano_escolar in Ano_escolar_dados:
        ...
        Ano_escolar[Ano_escolar_id] = {
            ...
            'media_geral': Ano_escolar[4],  # media_geral
            'escolas': []
        }
...
def relatorio_turma():
    ...
    rendered_html = render_template(
        ...
        turma={"Ano_escolar": turma[1], "turma": turma[2]},
        desempenho=desempenho,
        parecer=None,
        error=None,
